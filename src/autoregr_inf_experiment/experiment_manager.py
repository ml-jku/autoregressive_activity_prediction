"""
This files include experiment manager modules.
...
from dataloader import (DataModule_for_autregressive_inference_on_FSMol_testset,
                        DataModule_for_transductive_inference_on_FSMol_testset)
...
class ExperimentManager_AutoregressiveInference():
    ...
    def perform_experiment(self):
        ...
        # Save results
        current_loc = __file__.rsplit("/",3)[0]
        with open(current_loc + self.cfg.results_path, 'wb') as f:
            pickle.dump([auc_dict, dauc_pr_dict], f)

class ExperimentManager_TransductiveInference():
    def __init__(self, cfg=Config()):
        self.model = MHNfs_inference_module(device=cfg.device)
        self.cfg = cfg
    ...
    def perform_experiment(self):
        ...
        # Save results
        current_loc = __file__.rsplit("/",3)[0]
        with open(current_loc + self.cfg.results_path_transductive, 'wb') as f:
            pickle.dump([auc_dict, dauc_pr_dict], f)

class ExperimentManager_AutoregressiveInference_baseline():
    def __init__(self, cfg=Config()):
        self.model = MHNfs_inference_module(device=cfg.device)
        self.cfg = cfg
    ...
    def perform_experiment(self):
        ...
        for task_id in self.task_ids:
            ...
            for seed in self.experiment_rerun_seeds:
                ...
                for i in range(self.cfg.nbr_support_set_candidates):
                    ...
                    # Update support set
                    raw_active_cand_id, raw_inactive_cand_id = np.random.choice(
                    len(candidate_predictions), 2, replace=False)
        ...
        # Save results
        current_loc = __file__.rsplit("/",3)[0]
        with open(current_loc + self.cfg.results_path_baseline, 'wb') as f:
            pickle.dump([auc_dict, dauc_pr_dict], f)
...
if __name__ == '__main__':
    ...
    # Run experiment with random picking policy
    #experiment_manager = ExperimentManager_AutoregressiveInference_baseline()
    
    # Experiment variation: Query set == candidate set
    #experiment_manager = ExperimentManager_TransductiveInference()
